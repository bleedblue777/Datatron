(function($) {
    ...
    $('#edit-search-name').keydown(function(e) {
        var key = e.which;
        if (key == 37) // the left key code
        {
            $('#prev-edit').click();
    ...
    $('#search-by-name-button').click(function() {
        var input_first_name = $('#search-by-name-firstname').val().trim();
        var input_middle_name = $('#search-by-name-middlename').val().trim();
        var input_last_name = $('#search-by-name-lastname').val().trim();

        if (alphanumeric(input_first_name) || alphanumeric(input_middle_name) || alphanumeric(input_last_name)) {
            ...
            if (input_last_name.search("'") != -1) {
                input_last_name = input_last_name.substring(0, input_last_name.search("'")) + "'" + input_last_name.substring(input_last_name.search("'"), input_last_name.length);
            }
        ...
        WHERE a.` + ac_details.first_name_en + ` like '` + input_first_name + `%'
        AND a.` + ac_details.rln_fm_nm_en + ` like '` + input_middle_name + `%'
        AND a.` + ac_details.last_name_en + ` like '` + input_last_name + `%'`;
            ...
            $('input.editor-active1', row).prop( 'checked', data.active == 1 );
                ...
                name_table = $('#bootstrap-data-table').DataTable();
                name_table.columns.adjust().draw();
    ...
    $('#name-table-content').on('dblclick', 'tr', function() {
        if(name_table.row($(this).prev()).data()){
      $('#prev').show();
    }
    else{
       $('#prev').hide();
    }
    if(name_table.row($(this).next()).data()){
      $('#next').show();
    }
    else{
       $('#next').hide();
        ...
        console.log(name_table.row(this).data());
        table_row = name_table.row(this).data();
        var t = '<tbody>';
    ...
    $('#family-btn-namewise').click(function() {
        $('#closemodal1').click();
        $('#name-table-div').hide();
        $('#loading_gif').show();
        $('#back-btn').show();
    ...
    $('#edit-button').click(function() {
        $('#exampleModalCenter').hide();
        $('#edit-search-name').show();
        $('#edit-search-name').modal({
            backdrop: false,
            show: true
        });
    ...
    $('#prev').click(function() {
        ...
        if (name_table.row(prev_tr).data()) {

            $('#next').show();

            table_row = name_table.row(prev_tr).data();
            var t = '<tbody>';
            var keys = Object.keys(table_row);
    ...
    $('#next').click(function() {
        ...
        if (name_table.row(next_tr).data()) {

            $('#prev').show();

            table_row = name_table.row(next_tr).data();
            var t = '<tbody>';
            var keys = Object.keys(table_row);
    ...
    function alphanumeric(inputtxt) {
        if (inputtxt.match(/^[a-zA-Z\s']*$/) && inputtxt != '') {
            return true;
        } else {
            return false;
        }
