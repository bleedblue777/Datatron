(function ($) {
  ...
  var save_voter_curr = 0;
  var save_voter_prev = -1;
  var save_voter_next = 1;
  ...
  $('#exampleModalCenter').keydown(function(e) {
    ...
    $('#edit-voter-selection').keydown(function(e) {
        var key = e.which;
        if (key == 37) // the left key code
        {
            $('#prev-edit').click();
    ...
    $('#voter-selection-confirmation').keydown(function(e) {
        var key = e.which;
        if (key == 37) // the left key code
        {
            $('#prev-confirmation').click();
            return false;
        }
        else if (key == 39) // the right key code
        {
            $('#next-confirmation').click();
            return false;
        }
    });

  $('#voter-selection-button').click(function(){
    ...
    var query = `SELECT a.` + ac_details.ac_no + ` as ac_no
        ...
        ,'' as active
        ,a.` + ac_details.voter_status + ` as voter_status
    ...
    if(min_filter_for_query >= 1 || filter_flag == 1){
      ...
      $('#voter-selection-table-div').hide();
      $('#loading_gif').show();
      ...
      const request = pool.request();
        ...
        // ... error checks
              ...
              $('#voter-selection-table-div').show();
              ...
              $('#bootstrap-data-table').DataTable( {
                    ...
                    columns: [
                    {
                        ...
                        render: function ( data, type, row ) {
                            if ( type === 'display' ) {
                                return `<input type="checkbox" id="editor-active1" name = "A" style="background:#3cba9f">
                                <input type="checkbox" id="editor-active2" name = "B" style="background:#ff0000">
                                <input type="checkbox" id="editor-active3" name = "C" style="background:#3e95cd">
                                <input type="checkbox" id="editor-active4" name = "D" style="background:#e8c3b9">`;
                            }
                            return data;
                        },
              ...
              voter_selection_table = $('#bootstrap-data-table').DataTable();
              voter_selection_table.columns.adjust().draw();

              SetClassForCheckboxes();
  ...
  $('#voter-selection-table-content').on('dblclick','tr',function(e){
    if($(e.target).is('input')){
      return;
    }

    if(voter_selection_table.row($(this).prev()).data()){
      $('#prev').show();
    }
    else{
       $('#prev').hide();
    }
    if(voter_selection_table.row($(this).next()).data()){
      $('#next').show();
    }
    else{
       $('#next').hide();
    ...
    $('#exampleModalCenter').show()
        ...
        $('#modal-drag-voter-selection').draggable({
            handle: ".modal-header"
        });

    console.log(voter_selection_table.row(this).data());
    table_row = voter_selection_table.row(this).data();
    var t = '<tbody>';
  ...
  $('#voter-selection-save-button').on('click',function(){

    var selected_option = '';
    var row_count = voter_selection_table.data().count();

    save_voter_curr = 0;
    save_voter_prev = -1;
    save_voter_next = 1;

    if(save_voter_prev >= 0){
      $('#prev-confirmation').show();
    }
    else{
       $('#prev-confirmation').hide();
    }
    if(save_voter_next < row_count){
      $('#next-confirmation').show();
    }
    else{
       $('#next-confirmation').hide();
    }

    $('#voter-selection-confirmation').show()
    
    $('#voter-selection-confirmation').modal({
        backdrop: false,
        show: true
    });

    $('#modal-drag-voter-confirmation').draggable({
        handle: ".modal-header"
    });

    table_row = voter_selection_table.row(save_voter_curr).data();
    var t = '<tbody>';
    var keys = Object.keys(table_row);
    ...
    t += '<tr><th style="color:red">Old Status</th><td>';
    t += table_row["voter_status"];
    t += '</td></th>';
    t += '<th style="color:green">New Status</th><td>';

    selected_option = table_row["voter_status"];

    $('tr:eq(2)').find('input[type="checkbox"]').each(function() {
      if($(this).prop("checked")) {
        selected_option = $(this).attr('name');
      }
    });
    ...
    $('#modal-status-confirm-id').html(t);
    save_voter_prev = save_voter_curr - 1;
    save_voter_next = save_voter_curr + 1;
  });
  ...
  $('#prev').click(function() {
        ...
        if (voter_selection_table.row(prev_tr).data()) {

            $('#next').show();

            table_row = voter_selection_table.row(prev_tr).data();
            var t = '<tbody>';
            var keys = Object.keys(table_row);
    ...
    $('#next').click(function() {
        ...
        if (voter_selection_table.row(next_tr).data()) {

            $('#prev').show();

            table_row = voter_selection_table.row(next_tr).data();
            var t = '<tbody>';
            var keys = Object.keys(table_row);
    ...
    $('#prev-confirmation').click(function() {
      ...
      if (save_voter_prev >= 0){
        save_voter_prev = save_voter_prev - 1;
        save_voter_curr = save_voter_curr - 1;
        save_voter_next = save_voter_next - 1;
        selected_row = save_voter_curr + 2;
      

        if (voter_selection_table.row(save_voter_curr).data()) {
          ...
          if(save_voter_next < row_count)
            ...
            $('tr:eq('+selected_row+')').find('input[type="checkbox"]').each(function() {
              if($(this).prop("checked")) {
                selected_option = $(this).attr('name');
              }
            });
    ...
    $('#edit-button').click(function() {
        ...
        $('#edit-search-voter-selection').modal('show');
        $('#edit-search-voter-selection').modal({
            backdrop: false,
            show: true
        });
        $('#edit-search-voter-selection').draggable({
            handle: ".modal-header"
        });
...
function alphanumeric(inputtxt)
{
 if(inputtxt.match(/^[0-9a-zA-Z//]+$/)) 
  ...
  function getVoterSelectionDataForFilter() {
  ...
  getVoterSelectionDataForFilter();
  ...
  $('#confirm-selection-btn').on("click", function() {
    ...
    $('#voter-selection-table-content').find('tr').each(function() {
        var A_list = "";
        var B_list = "";
        var C_list = "";
        var D_list = "";
        var table_row_voter_selection = voter_selection_table.row(this).data();
        var ac_no_to_update = table_row_voter_selection["ac_no"];
        var part_no_to_update = table_row_voter_selection["part_no"];
        var serial_no_to_update = table_row_voter_selection["serial_no"];

        $(this).find('#editor-active1').each(function() {
          A_list = this.checked ? "A" : "";
          if(A_list == "A"){
            updateVoterSelection(A_list,ac_no_to_update,part_no_to_update,serial_no_to_update);
            cnt++;
          }
        });

        $(this).find('#editor-active2').each(function() {
          B_list = this.checked ? "B" : "";
          if(B_list == "B"){
            updateVoterSelection(B_list,ac_no_to_update,part_no_to_update,serial_no_to_update);
            cnt++;
          }
        });

        $(this).find('#editor-active3').each(function() {
          C_list = this.checked ? "C" : "";
          if(C_list == "C"){
            updateVoterSelection(C_list,ac_no_to_update,part_no_to_update,serial_no_to_update);
            cnt++;
          }
        });

        $(this).find('#editor-active4').each(function() {
          D_list = this.checked ? "D" : "";
          if(D_list == "D"){
            updateVoterSelection(D_list,ac_no_to_update,part_no_to_update,serial_no_to_update);
            cnt++;
          }
        });

        console.log(A_list);
        console.log(ac_no_to_update);
        console.log(part_no_to_update);
        console.log(serial_no_to_update);
    });
    if(cnt > 0)
      window.alert(cnt+" Voters Updated");
  });

  function updateVoterSelection(status,ac_no_to_update,part_no_to_update,serial_no_to_update){

    const request = pool.request();

    var query = `update [` + ac_details.ac_db + `].[dbo].[` + ac_details.ac_table + `]
                set ` + ac_details.voter_status + ` = '` + status + `'
                where ` + ac_details.ac_no + ` = ` + ac_no_to_update + `
                and ` + ac_details.part_no + ` = ` + part_no_to_update + `
                and ` + ac_details.serial_no + ` = ` + serial_no_to_update;
  ...
  function SetClassForCheckboxes(){
    var classcount = 1;
    var i = 1;
    $(voter_selection_table.$("input")).each(function(){
      var classname = "class"+classcount;
  ...
  $(document).on('click', '[type=checkbox]', function (e) {
    ...
    $('.'+class_name).not(this).prop('checked', false);
  });
