(function($) {
    ...
    $('#edit-search-address').keydown(function(e) {
        var key = e.which;
        if (key == 37) // the left key code
        {
            $('#prev-edit').click();
    ...
    $('.input-form').keypress(function(e) {
        ...
        {
            $('#search-by-address-button').click();
            return false;
        }
    });
    ...
    $('#search-by-address-button').click(function() {
        var input_part_no = $('#search-by-address-part-no').val().trim();
        var input_address = $('#search-by-address-address').val().trim();


        if (input_part_no == '' && input_address == ''){
            window.alert("Please enter more filters. Data too large to display");
            return;
        }

        if (alphanumeric(input_part_no) || alphanumeric(input_address)) {

            $('#loading_gif').show();
            $('#address-table-div').hide();

            const request = pool.request();

            if (input_address.search("'") != -1) {
                input_address = input_address.substring(0, input_address.search("'")) + "'" + input_address.substring(input_address.search("'"), input_address.length);
            }
        ...
        if(input_part_no != '')
        {
            query += `AND a.` + ac_details.part_no + ` = ` + input_part_no;
        }

        if(input_address != '')
        {
            query += `AND isnull(trim(a.` + ac_details.house_no_en + `),'') + case when isnull(trim(a.` + ac_details.house_no_en + `),'') = '' then '' else ', ' end + isnull(trim(b.` + sec_details.section_name + `),'') +', '+ isnull(trim(b.` + sec_details.pin_code + `),'') like '%` + input_address + `%'`;
        }
            ...
            request.query(query, (err, result) => {
                ...
                $('#bootstrap-data-table').DataTable({
                    ...
                    //             <input type="checkbox" class="editor-active2">
                    //             <input type="checkbox" class="editor-active3">`;
                    //         }
                    //         return data;
                    //     },
            ...
            $('input.editor-active1', row).prop( 'checked', data.active == 1 );
        }
                });
                $('#loading_gif').hide();

                address_table = $('#bootstrap-data-table').DataTable();
                address_table.columns.adjust().draw();

                $('body').addClass('open');

            });
        } else
            window.alert("Invalid Input. Please Enter Alphabet Characters Only");
    ...
    $('#address-table-content').on('dblclick', 'tr', function() {
        if(address_table.row($(this).prev()).data()){
      $('#prev').show();
    }
    else{
       $('#prev').hide();
    }
    if(address_table.row($(this).next()).data()){
      $('#next').show();
    }
    else{
       $('#next').hide();
        ...
        $('#modal-drag-addresswise').draggable({
            handle: ".modal-header"
        });

        console.log(address_table.row(this).data());
        table_row = address_table.row(this).data();
        var t = '<tbody>';
    ...
    $('#family-btn-addresswise').click(function() {
        $('#closemodal1').click();
        $('#address-table-div').hide();
        $('#loading_gif').show();
        $('#back-btn').show();
    ...
    $('#edit-button').click(function() {
        $('#exampleModalCenter').hide();
        $('#edit-search-address').show();
        $('#edit-search-address').modal({
            backdrop: false,
            show: true
        });
    ...
    $('#prev').click(function() {
        ...
        if (address_table.row(prev_tr).data()) {

            $('#next').show();

            table_row = address_table.row(prev_tr).data();
            var t = '<tbody>';
            var keys = Object.keys(table_row);
    ...
    $('#next').click(function() {
        ...
        if (address_table.row(next_tr).data()) {

            $('#prev').show();

            table_row = address_table.row(next_tr).data();
            var t = '<tbody>';
            var keys = Object.keys(table_row);
