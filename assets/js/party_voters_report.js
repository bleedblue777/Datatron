(function ($) {
  ...
  function getPartyVoters() {
        ...
        // ... error checks
            if (err) {
              ...
              party_voters_table = $('#bootstrap-data-table').DataTable();
  ...
  function getFamilyDetailsForPerson() {
        ...
        WHERE a.`+ac_details.section_no+` = `+person_section_no+` AND a.`+ac_details.house_no_en+` = '`+person_house_no+`' AND a.`+ac_details.house_no_en+` <> '' AND a.`+ac_details.part_no+` = `+person_part_no;
          ...
          // ... error checks
              ...
              party_voters_table = $('#bootstrap-data-table').DataTable();
              party_voters_table.columns.adjust().draw();
  ...
  $('#party-voters-table-content').on('dblclick','tr',function(){
    if(party_voters_table.row($(this).prev()).data()){
      $('#prev').show();
    }
    else{
       $('#prev').hide();
    }
    if(party_voters_table.row($(this).next()).data()){
      $('#next').show();
    }
    else{
       $('#next').hide();
    }
    console.log(party_voters_table.row(this).data());
    table_row = party_voters_table.row(this).data();
    var t = '<tbody>';
  ...
  $('#family-btn-party-voters').click(function(){
    $('#closemodal1').click();
    $('#party-voters-table-div').hide();
    $('#loading_gif').show();
  ...
  $('#edit-button').click(function() {
        $('#exampleModalCenter').hide();
        $('#edit-party-voters').show();
        $('#edit-party-voters').modal({
            backdrop: false,
            show: true
        });
    ...
    $('#prev').click(function() {
        ...
        if (party_voters_table.row(prev_tr).data()) {

            $('#next').show();

            table_row = party_voters_table.row(prev_tr).data();
            var t = '<tbody>';
            var keys = Object.keys(table_row);
    ...
    $('#next').click(function() {
        ...
        if (party_voters_table.row(next_tr).data()) {

            $('#prev').show();

            table_row = party_voters_table.row(next_tr).data();
            var t = '<tbody>';
            var keys = Object.keys(table_row);
    ...
    $('#edit-party-voters').keydown(function(e) {
        var key = e.which;
        if (key == 37) // the left key code
        {
            $('#prev-edit').click();
